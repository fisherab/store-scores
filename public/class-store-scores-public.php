...
class Store_Scores_Public {
    ...
    public function enqueue_styles() {
         ...
         * defined in Store_Scores_Loader as all of the hooks are defined in
         * that particular class.
         * The Store_Scores_Loader will then create the relationship between the
    ...
    public function enqueue_scripts() {
         ...
         * defined hooks and the functions defined in this class.
         */
        wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/store-scores-public.js', array(
            'jquery'
        ), $this->version, false);
    }
    ...
    private function register_short_codes() {
        add_shortcode('ss-get-description', [
            $this,
            'get_description'
        ]);
        add_shortcode('ss-enter-score', [
            $this,
            'enter_score'
        ]);
        add_shortcode('ss-list-scores', [
            $this,
            'list_scores'
        ]);
        add_shortcode('ss-show-results', [
            $this,
            'show_results'
        ]);
    }
    ...
    public function enter_score($atts, $content = null) {
        ...
        if ($me->ID === 0)
            return 'Sorry, you must be logged in to enter a result.';
        $submitter_id = $me->ID;
        if (array_key_exists('id', $atts)) {
            $comp_id = $atts['id'];
            if (get_post_type($comp_id) !== 'ss_competition') {
            ...
            if (! in_array($me->ID, $competitors) && ! $tman)
                return 'Sorry, you are not competing in this event';
        } else {
            return 'Competion not specified in call to short code.';
        }
        ...
        if (isset($_GET['comp_id']) && ($_GET['comp_id'] == $comp_id)) {
            if (isset($_GET['fail'])) {
                ...
                switch ($fail) {
                    ...
                    case 'missedtarget':
                        $message = 'Exactly one player must achieve the target score';
                        break;
                        
                    case 'exceededtarget':
                        $message = 'You cannot exceed a target score';
                        break;
                        
                    case 'noopposingscore':
                        $message = 'You must record an opposing score';
                        break;
                        
                    case 'not3missed':
                            $message = 'Only the third game can be ommitted';
                            break;
    ...
    public function list_scores($atts, $content = null) {
        $me = wp_get_current_user();
        if ($me->ID === 0)
            return 'Sorry, you must be logged in to work with results.';
        $submitter_id = $me->ID;
        if (array_key_exists('id', $atts)) {
            $comp_id = $atts['id'];
            if (get_post_type($comp_id) !== 'ss_competition') {
        ...
        if (isset($presults)) {
            ...
            foreach ($presults as $presult) {
                ...
                foreach ($presult['scores'][0] as $n => $v) {
                    $html .= '<td>' . $presult['scores'][0][$n] . $presult['scores'][1] . '-' . $presult['scores'][2][$n] . $presult['scores'][3] . '</td>';
                }
                $html .= '</tr>';
            }
            $html .= '</table>';
    ...
    public function process_submit_score() {
        ...
        if ($targetscores) {
            $youtarget = $_POST['youtarget'];
            $opptarget = $_POST['opptarget'];
            ...
            for ($i = 1; $i <= $bestof; $i ++) {
                if ($you[$i] == "" && $opp[$i] == "") {
                    ...
                    $fail = 'noopposingscore';
                }
            }
             
            // Check that nobody has exceeded target
            if (! $fail) foreach ($you as $y) if ($y > $youtarget) $fail = 'exceededtarget';
            if (! $fail) foreach ($opp as $o) if ($o > $opptarget) $fail = 'exceededtarget';
            if (! $fail) {
                if ($bestof == 1) {
                    // Check that only one person has reached target
                    if (! ($you[1] == $youtarget) xor ($opp[1] == $opptarget)) $fail = 'missedtarget';
                } elseif ($bestof == 3) {
                    for ($i = 1; $i <= count($you); $i ++) {
                        // Check that only one person has reached target
                        if (! ($you[$i] == $youtarget) xor ($opp[$i] == $opptarget)) $fail = 'missedtarget';
                    }
                    if (! $fail) {
                        // Check that games one and two have different winners
                        // if there is a third game
                        if (count($you) == 3){
                            if (! ($you[1] == $youtarget) xor ($you[2] == $youtarget)) $fail = 'extra';
                        } else {
                            // No third game so one and two must have same winner
                            if (! ($you[1] == $youtarget) === ($you[2] == $youtarget)) $fail = 'nodraws';
                        }
                    }
                }
            }
            
        } else { // Not target scores
            if ($bestof == 1) {
                if ($you[1] == $opp[1]) $fail = 'nodraws';
            } elseif ($bestof == 3) {
                if ($you[1] == $opp[1]) $fail = 'nodraws';
                if (! $fail) {
                    if ($you[2] == $opp[2]) $fail = 'nodraws';
                    if (! $fail) {
                        $need3 = (($you[1] > $opp[1]) && ($you[2] < $opp[2]) || ($you[1] < $opp[1]) && ($you[2] > $opp[2]));
                        if (! $need3) {
                            if (($you[3] != 0) || ($opp[3] != 0)) $fail = 'extra';
                        } else {
                            if ($you[3] == $opp[3]) $fail = 'nodraws';
                        }
                    }
                }
            }
        ...
        if ($fail) {
            ...
            if ($targetscores) {
                $result['you'] = [
                    'person' => $_POST['you_id'],
                    'target' => $_POST['youtarget'],
                    'scores' => $you
                ];
                $result['opp'] = [
                    'person' => $_POST['opp_id'],
                    'target' => $_POST['opptarget'],
                    'scores' => $opp
                ];
            } else {
                $result['you'] = [
